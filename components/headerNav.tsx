...
export default function HeaderNav() {
    ...
    return (
        <header className="w-full bg-[var(--surface)] text-[var(--foreground)] flex flex-row fixed py-3 gap-4 justify-items-center justify-center z-50 shadow-sm">
            <button type="button" onClick={navigation} aria-label={t.home} className="flex-none w-fit h-full ml-10 cursor-pointer bg-transparent border-0 p-0">
                <svg xmlns="http://www.w3.org/2000/svg" width="131" height="50" viewBox="0 0 131 27" aria-hidden="true" focusable="false">
            ...
            <Button href="/cart" type="text" aria-label={`${t.cart} (1)`} style={{fontSize: '20px', width:'40px', height:'100%' ,color:'#3AC5C9' }} icon={<ShoppingCartOutlined />} >
            </Button>
            <span aria-hidden="true" className="text-xs bg-red-600 relative rounded w-full text-white  bottom-3 right-4 text-center" style={{width:'15px' ,borderRadius:'50px'}}  >1</span>
            </div>
